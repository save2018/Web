...
function StartCapture()
{
    ...
    $('#imgshow').hide();  
	$('#imgshow').attr('src', "./images/loading.gif?v=1");
    ...
    {		
		...
		$('#info').show();
		$('#info').html('正在截图中, 如果超过5秒无响应，请点此<a target="_blank" href="' + downloadUrl + '" + date.getMinutes() + date.getSeconds()">安装</a>截图插件,安装后请刷新浏览器');
	}
}
...
function OnCaptureFinishedCallback(type, x, y, width, height, info, content,localpath) 
{
    ...
    switch(type)
    {
        case 1:
        {
        	...
        	$("#show_img").attr('src',"data:image/png;base64,"+content);
        	$("#base_id").val(content);
...
function UploadData()
{
    ...
    var data = "act=jietu&userid=test111&filename=" + filename + "&picdata=" + encodeURIComponent(savedPictureContent);	
             
    $.ajax({
            type: "POST",
            url: "./up.php",  //upload.ashx
            dataType: "json",
            data: data, 

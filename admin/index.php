...
if ($rec == 'default') {
    ...
    //注册商店总数
    $store_total=$dou->num_rows($dou->query("SELECT * FROM " . $dou->table('store')));
    $store_a=$dou->num_rows($dou->query("SELECT * FROM " . $dou->table('store')." where auditing_status=1"));
    //echo $store_total;
    $smarty->assign('store_total', $store_total);
    $smarty->assign('store_a', $store_a);
    
    //查询 代理商总数
    $user_total=$dou->num_rows($dou->query("SELECT * FROM " . $dou->table('user')));
    ...
    //
    $open_line=$dou->get_one("SELECT value FROM " . $dou->table('config') . " where name='open_line'");
    $smarty->assign('open_line', $open_line);

    $show_shenhe=$dou->get_one("SELECT value FROM " . $dou->table('config') . " where name='show_shenhe'");
    $smarty->assign('show_shenhe', $show_shenhe);
 ...
 *ajax是否开启测距
 */
elseif ($rec=='open_line'){

    $val=trim($_POST['val']);
    $data['value']=$val;
    $dou->autoExecute('m_config',$data,"UPDATE","name='open_line'");
    echo  json_encode(array('msg'=>'ok'));
}
/**
 *ajax是否审核后显示店铺
 */
elseif ($rec=='show_shenhe'){

    $val=trim($_POST['val']);
    $data['value']=$val;
    $dou->autoExecute('m_config',$data,"UPDATE","name='show_shenhe'");
    echo  json_encode(array('msg'=>'ok'));
}
/**
 * +----------------------------------------------------------
